...
// Reusable gradient button classes to avoid Intellisense warnings
const gradientBtnBase = "text-white px-6 py-2 rounded-full shadow-md hover:shadow-lg transition-transform duration-300";
  ...
  return (
    <section id="lookbook" className="py-24 bg-gray-50 dark:bg-gray-900 relative overflow-hidden">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {lookbookItems.map((item) => (
            ...
            >
              ...
              />
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6 rounded-3xl">
                <h3 className="text-xl md:text-2xl font-bold text-white mb-2">{item.title}</h3>
                <p className="text-gray-200">{item.description}</p>
                <Link href="/shop">
                  <button className={gradientBtnShop}>Shop Now</button>
                </Link>
              </div>
            </div>
          ))}
        ...
        {/* Footwear Highlights (horizontal scrollable) */}
        <h2 className="text-4xl md:text-5xl font-extrabold text-center text-gray-900 dark:text-white mb-12">
          Footwear Highlights
        </h2>

        <div className="flex space-x-6 overflow-x-auto pb-4 scrollbar-hide mb-12">
          {footwearItems.map((item) => (
            <div
              key={item.id}
              className="min-w-[250px] relative overflow-hidden rounded-3xl shadow-2xl group cursor-pointer transform hover:scale-105 transition-transform duration-500"
            >
              <Image
                src={item.image}
                alt={item.title}
              ...
              />
              <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6 rounded-3xl">
      ...
      <div className="absolute top-0 left-1/4 w-72 h-72 bg-gradient-to-r from-purple-500 via-pink-400 to-yellow-400 rounded-full opacity-20 blur-3xl animate-float-slow pointer-events-none"></div>
      <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-gradient-to-r from-pink-500 via-purple-400 to-blue-400 rounded-full opacity-15 blur-3xl animate-float-slow-slow pointer-events-none"></div>
